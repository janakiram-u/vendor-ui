...
const VendorForm = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/vendors/categories');
  ...
  const handleCategoryChange = (event, value) => {
    ...
    const fetchServiceNames = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/vendors/serviceNames?category=${value}`);
