...
const VendorList = () => {
  ...
  const fetchVendors = async (page, rowsPerPage) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/vendors?page=${page}&limit=${rowsPerPage}`);
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`http://localhost:5000/api/vendors/${currentVendor._id}`, currentVendor);
      fetchVendors(page, rowsPerPage);
      handleCloseEdit();
    } catch (error) {
      console.error(error);
